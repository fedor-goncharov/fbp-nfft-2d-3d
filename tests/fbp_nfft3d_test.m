...
% make sinogram
r = 0.15;
sino_ball_func = @(x) (abs(x) < 0.15)*pi*(r^2 - x^2);
s_grid_size = 129;
...
% FBP in 3D
[nodes, values, jacobian_weights, Fs] = rtft3d(sinogram, ntheta, nphi, s_grid_size, 1.0, 0.5, true);
recon3d = nfft_reconstruct_3d(192, nodes, values, jacobian_weights, Fs);

% plot a slice through center
recon3d_slice = real(recon3d(:, :, 96));
figure, imagesc(recon3d_slice, [min(min(recon3d_slice)), max(max(recon3d_slice))]), colorbar;
figure, plot(linspace(-1,1,192), recon3d_slice(:, 96));

% TEST 2
% ball shifted from origin
...
figure, imagesc(squeeze(recon3d_slice), [0,1]), colorbar, ...
  title(sprintf("shift=[%2.2f %2.2f %2.2f]", shift(1), shift(2), shift(3)));

% image returned by NFFT is ordered as [z, y, x] with monotone indicies that correspond to (-1, 1)
